...
$properties += New-xDscResourceProperty -Name 'Roles' -Type String -Attribute Write -ValidateSet 'FrontEnd','WordBackEnd','ExcelBackEnd','PowerPointBackEnd' -Description 'Specifies one or more server roles, separated by commas, to assign to the new server. If no roles are specified, then the server is assigned all roles.'
